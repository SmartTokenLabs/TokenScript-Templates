	...
	{#if token}
		...
		<div style="background-color: white; border-radius: 7px; width: 100%; display: flex; justify-content: space-between; flex-direction: column; padding: 0 18px;">
			...
			{#if token?.tokenInfo?.attributes?.length > 0}
				<div style="margin-bottom: 18px; background-color: #F5F5F5; color: #989898; font-weight: 300; border-radius: 20px; padding: 12px 22px;">
					...
					{#each token?.tokenInfo?.attributes as trait}
						<div style="margin-bottom: 18px; width: 148px; background-color: white; font-size: 12px; text-align: center; border-radius: 20px; font-weight: 300; padding: 12px; border: 1px solid rgb(182, 182, 191);">
							<p style="font-size: 14px; color: rgb(136, 136, 136); font-weight: 400;">{trait.trait_type}</p>
			...
			<div style="margin-bottom: 18px; background-color: #F5F5F5; border-radius: 20px; font-weight: 300; padding: 24px;">
				<p style="color: rgb(112, 112, 112); font-weight: 500; font-size: 14px;">Details</p>
				...
				{#if token.contractAddress}
					...
					<p style="font-size: 14px; color: rgb(136, 136, 136); word-wrap: break-word;">
						{#if explorer}
						<div style="font-size: 14px; color:black; word-wrap: break-word;">{token.contractAddress}
							<a href="{explorer+"address/"+token.contractAddress}" target="_blank">
				...
				{#if token.chainId}
					...
					<p style="font-size: 14px; color: black; word-wrap: break-word;">{token.chainId}</p>
				{/if}
				{#if token.symbol}
					<p style="font-size: 14px; color: rgb(136, 136, 136); font-weight: 400;">Symbol</p>
					<p style="font-size: 14px; color: black; word-wrap: break-word;">{token.symbol}</p>
				{/if}				
				{#if creatorRoyaltiesForSale}
					<p style="font-size: 14px; color: rgb(136, 136, 136); font-weight: 400;">Creator Earnings Per Sale</p>
					<p style="font-size: 14px; color: black; word-wrap: break-word;">{ creatorRoyaltiesForSale+'%' }</p>
				{/if}
				{#if token.external_link}
					<p style="font-size: 14px; color: rgb(136, 136, 136); font-weight: 400;">Website</p>
					<p style="font-size: 14px; color: black; word-wrap: break-word;">{token.external_link}</p>
				{/if}
			</div>
		</div>
	{/if}
