<script lang="ts">
	...
	async function setTokenPriceData() {
		const tokenStatsRequest = await fetch(
			`https://api.token-discovery.tokenscript.org/get-token-price?blockchain=evm&smartContract=0x6b175474e89094c44da98b954eedeac495271d0f&chain=eth`
		);
		...
		<div id="token-wrapper" class="primary-font-color">
			<div class="card-header-background">
				<div class="text-center text-2xl pt-12 pb-6 text-[#385bd2] flex justify-center">
					<img
						style="width: 130px"
						alt="dai logo"
						src="https://cdn.jsdelivr.net/gh/SmartTokenLabs/resources/images/logos/dai-logo-with-text.png"
					/>
				</div>
			...
			<div class="p-6 secondary-background-color">
				...
				<div class="mt-4 text-white">
					<div class="text-sm tertiary-font-color">Ethereum</div>
					<div class="text-lg font-semibold">
						{tokenStatsPriceEth ? tokenStatsPriceEth.toString().substring(0, 6) + ' ETH' : '-'}
					</div>
				</div>
				<div class="mt-4 text-white">
					<div class="text-sm tertiary-font-color">USD</div>
					<div class="text-lg font-semibold">
						{tokenStatsPriceUsd
